...
export const DraftCarouselStructureSchema = z.object({
  topic: z.string().describe('Тема карусели (например: "5 способов победить прокрастинацию")'),
  slideCount: z.number().min(3).max(15).default(7).describe('Количество слайдов (3-15, рекомендуется 7-10)'),
...
export function draftCarouselStructure(input: DraftCarouselStructureInput): string {
  ...
  const structureExamples: Record<string, string[]> = {
    educational: [
