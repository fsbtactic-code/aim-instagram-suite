...
function buildCSS(input: CreateStyleInput): string {
  ...
  // Запрашиваем веса 400, 700, 900 для нормального рендера жирного текста и поддержки кириллицы
  const googleFonts = font.fonts.map(f => `${f.replace(/ /g, '+')}:wght@400;700;900`).join('&family=');

  // Генерируем SVG паттерн для фона
  const bgPattern = getBackgroundPattern(input.mood ?? 'luxury', primary, secondary);

  return `/* AIM Custom Style: ${input.styleName ?? 'My Style'} */
/* Generated by aim_create_style | Mood: ${mood.label} | Font: ${font.label} */

@import url('https://fonts.googleapis.com/css2?family=${googleFonts}&display=swap&subset=cyrillic');
...
/* Накладываем эстетичный SVG-паттерн */
.slide::before {
  content: '';
  position: absolute;
...
/* Защищаем контент от перекрытия паттерном */
.slide > *:not(.slide-number):not(.cta-banner) {
  position: relative;
  z-index: 1;
}
...
.slide-title, h1, h2 {
  ...
  font-weight: ${input.fontStyle === 'bold' ? '900' : '700'} !important;
  font-size: var(--font-size-title) !important;
  line-height: 1.1 !important;
  letter-spacing: ${input.fontStyle === 'mono' ? '0.05em' : '-0.02em'} !important;
  text-transform: none !important; /* Убираем баг Neo-Brutalism, где всё капсом */
}

.slide-body, p, .check-text, .step-body, .cmp-cell {
  font-size: var(--font-size-body) !important;
  color: var(--color-text-muted) !important;
  line-height: ${input.contentDensity === 'rich' ? '1.8' : '1.6'} !important;
}
...
function getBackgroundPattern(mood: string, primaryHex: string, secondaryHex: string): string {
  const pColor = encodeURIComponent(primaryHex);
  const sColor = encodeURIComponent(secondaryHex);
