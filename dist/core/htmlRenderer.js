...
const designSystem_js_1 = require("./designSystem.js");
const slideLayouts_js_1 = require("./slideLayouts.js");
const FORMATS = {
...
// ─────────────────────────────────────────────────────────────────────────────
function getThemeDecorators(themeName) {
    switch (themeName) {
        case 'glassmorphism':
            return `
...
async function renderCarousel(slides, options) {
    ...
    let css = (0, designSystem_js_1.resolveThemeCSS)(theme);
    css += '\n' + slideLayouts_js_1.LAYOUTS_CSS;
    if (brandColorOverlay)
        css += '\n' + brandColorOverlay;
    if (customCssOverlay)
        css += '\n' + customCssOverlay;
    ...
    try {
        for (let i = 0; i < slides.length; i++) {
            ...
            const tmpPath = path.join(os.tmpdir(), `aim_slide_${(0, crypto_1.randomUUID)()}.html`);
...
// HTML BUILDER
// ─────────────────────────────────────────────────────────────────────────────
function buildSlideHTML(slide, css, theme, width, height, totalSlides) {
    const base = `
...
<body>${slide.customHtml}</body>
    ...
    const content = (0, slideLayouts_js_1.renderSlideLayout)(slide, totalSlides);
    return `<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
