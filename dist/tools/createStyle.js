...
function getDefaultPrimary(mood) {
    const map = {
...
function getDefaultSecondary(mood) {
    const map = {
...
function getBackground(base, mood) {
    if (base === 'light')
        return { main: '#FAFAF9', card: '#FFFFFF' };
    if (base === 'contrast')
        return { main: '#000000', card: '#111111' };
    // dark (default)
    const moodBg = {
...
function getTextColor(base) {
    if (base === 'light')
        return { main: '#1A1A1A', muted: '#6B6B6B' };
    return { main: '#F0F0F0', muted: '#9CA3AF' };
}
function getShadow(mood, primary) {
    if (mood === 'luxury')
        return `0 8px 32px rgba(0,0,0,0.6)`;
    if (mood === 'energetic')
        return `4px 4px 0px ${primary}`;
    if (mood === 'dark')
        return `0 0 20px ${primary}40`;
    if (mood === 'playful')
        return `6px 6px 0px ${primary}`;
    return `0 4px 16px rgba(0,0,0,0.2)`;
}
// ── Генератор SVG паттернов ─────────────────────────────────────────────────
function getBackgroundPattern(mood, primaryHex, secondaryHex) {
...
async function createStyle(input) {
    ...
    if (step === 2) {
        if (!input.mood)
            return JSON.stringify({ error: 'Укажи mood из шага 1' });
...
function getRecommendedLayout(input) {
    const layouts = ['standard'];
    if (input.contentDensity === 'minimal')
        layouts.push('hero-number', 'quote');
    if (input.contentDensity === 'rich')
        layouts.push('checklist', 'comparison');
    if (input.mood === 'energetic' || input.mood === 'playful')
        layouts.push('good-bad', 'before-after');
