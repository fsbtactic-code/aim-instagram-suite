...
(function bootstrap() {
    ...
    try {
        ...
        if (shelljs && shelljs.exec) {
            ...
            shelljs.exec = function (cmd, opts, cb) {
    ...
    process.stdout.write = function () {
        ...
        if (isMcpMsg) {
            return _ow.apply(process.stdout, args);
        }
...
const evaluateVideo_js_1 = require("./tools/evaluateVideo.js");
const analyzeViralReels_js_1 = require("./tools/analyzeViralReels.js");
const generateScript_js_1 = require("./tools/generateScript.js");
const analyzeHook_js_1 = require("./tools/analyzeHook.js");
const extractPacing_js_1 = require("./tools/extractPacing.js");
// ── Carousel Studio Tools ─────────────────────────────────────────────────────
const draftCarouselStructure_js_1 = require("./tools/draftCarouselStructure.js");
const renderPremiumCarousel_js_1 = require("./tools/renderPremiumCarousel.js");
const autoBrandColors_js_1 = require("./tools/autoBrandColors.js");
// ── Intelligence & Analysis Tools ─────────────────────────────────────────────
const scoreVirality_js_1 = require("./tools/scoreVirality.js");
const scoreCarouselVirality_js_1 = require("./tools/scoreCarouselVirality.js");
const analyzeCarousel_js_1 = require("./tools/analyzeCarousel.js");
const localizeCarousel_js_1 = require("./tools/localizeCarousel.js");
...
// ============================================================
const EvaluateVideoSchema = zod_1.z.object({
    videoPath: zod_1.z.string().describe('Абсолютный путь к локальному видеофайлу (mp4, mov, avi)'),
});
const AnalyzeViralReelsSchema = zod_1.z.object({
    url: zod_1.z.string().url().describe('Ссылка на Instagram Reel, TikTok или YouTube Shorts'),
    outputMdPath: zod_1.z.string().describe('Путь для сохранения .md отчёта (например: /Desktop/viral.md)'),
});
const GenerateScriptSchema = zod_1.z.object({
    referenceMdPath: zod_1.z.string().describe('Путь к .md файлу с отчётом aim_analyze_viral_reels'),
    targetTopic: zod_1.z.string().describe('Тема/ниша для нового сценария (например: "фитнес для занятых мам")'),
});
const AnalyzeHookSchema = zod_1.z.object({
    videoPath: zod_1.z.string().optional().describe('Путь к локальному видеофайлу'),
    url: zod_1.z.string().url().optional().describe('Ссылка на видео для анализа хука'),
}).refine(data => data.videoPath || data.url, {
    message: 'Укажите videoPath или url',
});
const ExtractPacingSchema = zod_1.z.object({
    videoPath: zod_1.z.string().optional().describe('Путь к локальному видеофайлу'),
    url: zod_1.z.string().url().optional().describe('Ссылка на видео для анализа ритма'),
    slowThresholdSec: zod_1.z.number().optional().default(4).describe('Порог провисания в секундах (default: 4)'),
}).refine(data => data.videoPath || data.url, {
    message: 'Укажите videoPath или url',
});
// ============================================================
...
// ============================================================
const DraftCarouselSchema = zod_1.z.object({
    topic: zod_1.z.string().describe('Тема карусели'),
    slideCount: zod_1.z.number().min(3).max(15).default(7).describe('Количество слайдов (3-15)'),
    toneOfVoice: zod_1.z.enum(['educational', 'motivational', 'professional', 'casual', 'provocative'])
        .default('educational').describe('Тон подачи'),
});
const RenderCarouselSchema = zod_1.z.object({
    slidesData: zod_1.z.union([zod_1.z.string(), zod_1.z.array(zod_1.z.any())]).describe('JSON-строка или массив слайдов'),
    theme: zod_1.z.union([
    ...
    format: zod_1.z.enum(['square', 'portrait']).default('square').describe('square=1080x1080, portrait=1080x1350'),
    outputDir: zod_1.z.string().describe('Папка для сохранения PNG'),
    globalCta: zod_1.z.string().optional().describe('Текст CTA-баннера'),
    brandColorOverlay: zod_1.z.string().optional().describe('CSS из aim_auto_brand_colors'),
    customCssOverlay: zod_1.z.string().optional().describe('Кастомный CSS'),
});
const BrandColorsSchema = zod_1.z.object({
    ...
    primaryColor: zod_1.z.string().regex(/^#[0-9A-Fa-f]{6}$/).describe('Основной цвет HEX'),
    secondaryColor: zod_1.z.string().regex(/^#[0-9A-Fa-f]{6}$/).describe('Вторичный цвет HEX'),
    textColor: zod_1.z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional().default('#FFFFFF').describe('Цвет текста HEX'),
});
// ============================================================
// Схемы входных параметров — Intelligence Tools
// ============================================================
const ScoreViralitySchema = zod_1.z.object({
    videoPath: zod_1.z.string().optional().describe('Путь к локальному видеофайлу'),
    url: zod_1.z.string().url().optional().describe('Ссылка на видео'),
    context: zod_1.z.string().optional().describe('Ниша и целевая аудитория (необязательно)'),
}).refine(data => data.videoPath || data.url, { message: 'Укажите videoPath или url' });
const ScoreCarouselViralitySchema = zod_1.z.object({
    url: zod_1.z.string().url().optional().describe('Ссылка на карусель Instagram'),
    slidesDir: zod_1.z.string().optional().describe('Путь к папке с PNG/JPG слайдами'),
    context: zod_1.z.string().optional().describe('Ниша, целевая аудитория'),
    goal: zod_1.z.enum(['sales', 'subscribers', 'reach', 'engagement', 'trust']).optional()
...
const AnalyzeCarouselSchema = zod_1.z.object({
    url: zod_1.z.string().url().describe('Ссылка на пост-карусель в Instagram/VK'),
    outputMdPath: zod_1.z.string().optional().describe('Путь для сохранения .md отчёта (необязательно)'),
});
const LocalizeCarouselSchema = zod_1.z.object({
    url: zod_1.z.string().url().describe('Ссылка на карусель для копирования/локализации'),
    mode: zod_1.z.enum(['copy', 'localize', 'adapt']).describe('copy=рерайт | localize=перевод на RU | adapt=адаптация под новую тему'),
    targetTopic: zod_1.z.string().optional().describe('Новая тема (только для mode=adapt)'),
    slideCount: zod_1.z.number().optional().describe('Количество слайдов в новой карусели'),
    designTheme: zod_1.z.number().min(1).max(8).optional().describe('Тема дизайна 1-8'),
    outputDir: zod_1.z.string().optional().describe('Папка для авторендера PNG (необязательно)'),
});
const CreateStyleSchema = zod_1.z.object({
    step: zod_1.z.union([
        zod_1.z.literal('1'), zod_1.z.literal('2'), zod_1.z.literal('3'), zod_1.z.literal('4'), zod_1.z.literal('5'),
        zod_1.z.literal(1), zod_1.z.literal(2), zod_1.z.literal(3), zod_1.z.literal(4), zod_1.z.literal(5),
    ]).transform(v => Number(v)).optional()
        .describe('Шаг мастера (1=настроение, 2=цвета, 3=шрифт, 4=подача, 5=генерация). Пропусти для старта.'),
    mood: zod_1.z.enum(['luxury', 'energetic', 'minimal', 'warm', 'dark', 'playful']).optional(),
    colorBase: zod_1.z.enum(['dark', 'light', 'contrast']).optional(),
    primaryHex: zod_1.z.string().optional().describe('Основной цвет HEX, например #C9A84C'),
    secondaryHex: zod_1.z.string().optional().describe('Вторичный цвет HEX'),
    fontStyle: zod_1.z.enum(['serif', 'modern', 'bold', 'handwritten', 'mono']).optional(),
    contentDensity: zod_1.z.enum(['minimal', 'balanced', 'rich']).optional(),
    emojiUsage: zod_1.z.enum(['none', 'subtle', 'expressive']).optional(),
    customWishes: zod_1.z.string().optional().describe('Дополнительные пожелания к стилю свободным текстом'),
...
const ViralStructureSchema = zod_1.z.object({
    ...
    topic: zod_1.z.string().optional().describe('Тема для адаптации шаблона'),
});
// ============================================================
...
// ============================================================
const server = new index_js_1.Server({ name: 'aim-instagram-suite', version: '1.2.0' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        switch (name) {
            ...
            case 'aim_viral_structure': {
                ...
                if (parsed.structureId) {
                    ...
                    const all = (0, viralStructures_js_1.listViralStructures)();
                    result = JSON.stringify({
                        tool: 'aim_viral_structure',
                        ...
                        availableFonts: (0, designSystem_js_1.listAvailableFonts)().length + ' кириллических шрифтов в библиотеке',
            ...
            case 'aim_create_style': {
                ...
                result = await (0, createStyle_js_1.createStyle)(parsed);
                break;
            }
            default:
                return {
        ...
        const extractGridImages = (obj) => {
            if (obj?.gridImages && Array.isArray(obj.gridImages)) {
                obj.gridImages.forEach((img) => {
                    if (img.base64) {
